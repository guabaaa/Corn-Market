            ...
            <ul id="pageInfo" class="pageInfo">
              ...
              <c:if test="${pageMaker.prev}">
                <li class="pageInfo_btn previous">
                  <a href="${path}/post/category?id=${list[0].category_id}&PageNum=${pageMaker.startPage-1}">Previous</a>
                </li>
              </c:if>

              <!-- 각 번호 페이지 버튼 -->
              <c:forEach var="num" begin="${pageMaker.startPage}" end="${pageMaker.endPage}">
                <li class="pageInfo_btn"><a href="${path}/post/category?id=${list[0].category_id}&PageNum=${num}">${num}</a></li>
              </c:forEach>

              <!-- 다음페이지 버튼 -->
              <c:if test="${pageMaker.next}">
                <li class="pageInfo_btn next"><a href="${path}/post/category?id=${list[0].category_id}&PageNum=${pageMaker.endPage + 1}">Next</a></li>
              </c:if>
            </ul>
          </div>
        </div>
